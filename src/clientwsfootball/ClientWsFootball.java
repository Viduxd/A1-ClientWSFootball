...
public class ClientWsFootball {
    ...
    public static void main(String[] args) {
        int yellowCard = yellowCardsTotal();
        System.out.println("Hay un total de " + yellowCard + " tarjetas amarillas.");
        
        System.out.println(allDefenders("spain").getString());
        
        List<String> countries = new ArrayList();
        for (int i = 0; i < countryNames(true).getTCountryInfo().size(); i++) {
            String c = countryNames(true).getTCountryInfo().get(i).getSName();
            System.out.println("Los defensas de "+ c + " son " + allDefenders(countryNames(true).getTCountryInfo().get(i).getSName()).getString());
        }
